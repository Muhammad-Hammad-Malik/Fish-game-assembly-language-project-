...
newmainstart:
        ...
        jnz l118
        mov ax, 0x0C04 ; put pixel in white color
        mov cx, 10 ; x position 200
        mov dx, 20 ; y position 200
        mov bx, 200
        mov di, 2 
        l1110:
                push cx
        ...
        jnz l1110
        mov ax, 0x0C07 ; put pixel in white color
        mov cx, 20 ; x position 200
        mov dx, 30 ; y position 200
        mov bx, 180
        mov di, 2 
        l1112:
                push cx
...
musicwalakaam: 
                delay2:          
                        ...
                        ; ax,bx,ip,cs,flags,cx storage area
...
tasktwo:
        ...
        call tone_7
        call tone_8
        call tone_1					; taking modulus by 4
...
gerrandom:
            ...
            mov ah, 2ch           ;Get time
            int 21h
            mov word[cs:random1],dx
            mov word[cs:random2],bx
            pop dx
            pop cx
            pop bx
            pop ax
...
green: 
        ...
        mov ax, 0xb800
        mov es, ax
        mov di, word[cs:greenplace]
        mov ax, word[cs:samundar]
        mov word[es:di], ax
        mov ax,word[cs:random1]
        inc ax
        mov cl,34
        mul cl
        sub ax, 10
        ...
        add dx,3040
        mov di,dx
        mov word[cs:greenplace], di
        mov word[es:di],0x2020
        mov byte[cs:greentick], 0
        pop di
        pop si
        pop dx
        pop cx
...
red: 
        ...
        mov ax, 0xb800
        mov es, ax
        mov di, word[cs:redplace]
        mov ax, word[cs:samundar]
        mov word[es:di], ax
        mov dh,0
        mov ax,word[cs:random2]
        inc ax
        mov cl,34
        mul cl
        sub ax, 10
        ...
        add dx,3040
        mov di,dx
        mov word[cs:redplace], di
        mov word[es:di],0x4020
        mov byte[cs:redtick], 0
        pop di
        pop si
        pop dx
        pop cx
...
;infinite loop
 infinte:
        mov byte[cs:exicution],0
        cmp byte[cs:stop],1
        jne noask
                                                handbrake:
                                                cmp byte[cs:exicution], 49
                                                je noescap
                                                cmp byte[cs:exicution], 21
                                                je escap
                                                jmp done
                                            noescap: 
                                                mov byte[cs:stop],0x00
                                                jmp done
                                            escap:  
                                                mov byte[cs:exit],1 
                                            done:  
